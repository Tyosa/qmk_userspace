...
bool process_record_num_word(uint16_t keycode, const keyrecord_t *record) {
    // If nothing happened for more than NUM_WORD_TIMER_MS, disable numword
    if (is_num_word_on && timer_elapsed(num_word_timer) > NUM_WORD_TIMER_MS) {
        disable_num_word();
        return true; // Let the usual process_record happen
    }
